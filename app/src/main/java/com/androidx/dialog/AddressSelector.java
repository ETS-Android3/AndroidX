...
public class AddressSelector {
    ...
    public static class Builder {
        ...
        public Builder(Context context) {
            ...
            jsonData = Json.parseJSONArray(IOProvider.readAssets(CoreApplication.app, "yunxiang.json"));
        }
        ...
        public Builder jsonName(String jsonName) {
            ...
            jsonData = Json.parseJSONArray(IOProvider.readAssets(CoreApplication.app, jsonName));
            return this;
        }
    ...
    private void createDialog() {
        ...
        for (int i = 0; i < Size.of(jsonData); i++) {
        ...
        cityData = Json.parseJSONArray(jsonData.get(selectedProvincePosition).get(JSON_KEY_CITIES));
        ...
        lv_province.setOnItemSelectedListener(new LoopView.OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                cityData = Json.parseJSONArray(jsonData.get(index).get(JSON_KEY_CITIES));
                setCityData(lv_city);
            }
        });
        lv_city.setOnItemSelectedListener(new LoopView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(int index) {
                areaData = Json.parseJSONArray(cityData.get(index).get(JSON_KEY_DISTRICTS));
                setAreaData(lv_area);
            }
        });
    ...
    private void setCityData(LoopView lv_city) {
        ...
        areaData = Json.parseJSONArray(cityData.get(selectedCityPosition).get(JSON_KEY_DISTRICTS));
        //如果只有一个，就不需要循环
        citySize = Size.of(cityList);
        if (citySize == 1) {
            lv_city.setNotLoop();
        } else {
            lv_city.setLoop();
    ...
    private void setAreaData(LoopView lv_area) {
        ...
        for (int i = 0; i < Size.of(areaData); i++) {
