...
public class UpdateService extends Service implements OnDownloadListener {
    ...
    public void downloadUpdateFile(String url) {
        downloader = new Downloader.Builder(getApplicationContext())
                .url(url).name(appName + ".apk")
                .listener(this)
                .build();
        downloader.start();
        Log.i(getResources().getString(R.string.app_name), this.getClass().getSimpleName() + " downloadUpdateFile url:" + url);
    }
