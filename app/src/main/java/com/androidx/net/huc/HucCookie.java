...
public class HucCookie {
    ...
    public void saveCookie(URL url, HttpURLConnection conn, int expires) {
        ...
        for (int i = 0; i < Size.of(cookieList); i++) {
            String expiresAt = cookieList.get(i).get("expiresAt");
            if (Long.parseLong(expiresAt) < new java.util.Date().getTime()) {
                cookieList.remove(i);
            }
        }
    ...
    public Map<String, String> decodeCookie(String cookie, int expires) {
        ...
        if (!TextUtils.isEmpty(cookie)) {
            ...
            map.put("expiresAt", new java.util.Date().getTime() + expires + "");
        }
        return map;
    }
    ...
    public long parseExpires(String expires) {
        ...
        return Time.parse(year + "-" + month + "-" + day + " " + timeSplit[1]).getTime();
    }
    ...
    public static List<Map<String, String>> getCookie(String host) {
        String cookieJson = DataStorage.with(CoreApplication.app).getString(host, "[]");
        List<Map<String, String>> cookieList = Json.parseJSONArray(cookieJson);
        return cookieList;
    }
    ...
    private void persistenceCookie(String host, List<Map<String, String>> cookieList) {
        ...
        if (cookieList == null || cookieList.size() == 0) {
            ...
            cookieJson = Json.parseMapList(cookieList);
        }
        Log.i(this.getClass().getSimpleName(), cookieJson);
        DataStorage.with(CoreApplication.app).put(host, cookieJson);
    }
    ...
    public static void remove(String host) {
        DataStorage.with(CoreApplication.app).put(host, "[]");
    }
